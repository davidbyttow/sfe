...
import com.simplethingsllc.store.client.AbstractEntityHandler;
...
import com.simplethingsllc.store.client.HandleWith;
import com.simplethingsllc.store.client.Ignored;
import com.simplethingsllc.store.client.Indexed;
import com.simplethingsllc.store.common.MoreObjects;
...
public class EntityMetadata {
  ...
  public static EntityMetadata fromType(Class<?> type, List<CompositeIndexDef> compositeIndexDefs) {
    ...
    if (handleWith != null) {
      handler = MoreObjects.newInstance(handleWith.value());
    }
    ...
    for (CompositeIndexDef def : compositeIndexDefs) {
      ...
      List<EntityPropField> props = new ArrayList<>(def.props.size());
      for (String propName : def.props) {
