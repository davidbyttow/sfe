...
import com.davidbyttow.sfe.common.Types;
...
/**
 * Implementation of {@link org.skife.jdbi.v2.BeanMapper} that we control, and can thus extend with new type support
 * (e.g. Joda, optionals, etc)
 *
 * @param <T>
 */
public class BetterBeanMapper<T> implements ResultSetMapper<T> {
  ...
  public static class Factory implements ResultSetMapperFactory {
    @Override public boolean accepts(Class type, StatementContext ctx) {
      return Types.isBean(type);
    }

    @SuppressWarnings("unchecked")
    @Override public ResultSetMapper mapperFor(Class type, StatementContext ctx) {
