...
import com.davidbyttow.sfe.testing.EntityStores;
import com.davidbyttow.sfe.testing.MysqlResource;
...
public class EntityStoreTest {
  ...
  private EntityStore createStore() throws IOException {
    DBI dbi = mysql.dbi();
    return EntityStores.createEntityStore(dbi);
  }

  @Test public void missingReturnsNull() throws Exception {
    EntityStore store = createStore();

    assertThat(store.get("asdfasdfasdf", Pod.class)).isNull();
  ...
  @Test public void querySingleProperty() throws Exception {
    ...
    assertThat(store.count(q)).isEqualTo(3);
